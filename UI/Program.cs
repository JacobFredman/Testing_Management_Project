        ...
        static void Main(string[] args)
        {
            while (true)
            {
                ...
                try {
                    ...
                    t1.Address = new Address("Jerusalem , Herzl St , 1");
                    t2.Address = new Address("בית שמש הרצל 631");
                    Thread.Sleep(9000);
                    Console.WriteLine(t1.Address.PlaceExist().ToString()+" "+t2.Address.PlaceExist().ToString());

                    
                      

                    t1.Scedual.AddHoursAllDays(10, 14, 18, 22);
                    t1.Scedual.AddHourToDay(Days.Sunday, 4, 6);
                    t1.Scedual[Days.Thursday].RemoveHours(11, 12);


                    Console.WriteLine(t1.ToString());
                    Console.WriteLine(t1.Scedual.ToString());
